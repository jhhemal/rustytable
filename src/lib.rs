...
impl Table {
    ...
    fn to_string(&self) -> String {
        ...
        table.push_str(&self.format_row(self.headers.clone(), true, None));
        for (i, row) in self.rows.iter().enumerate() {
            table.push_str(&self.format_row(
                row.iter().map(|cell| self.apply_theme(&cell.value, cell.color.clone(), cell.bold)).collect::<Vec<String>>(),
                false,
                self.theme.as_ref().and_then(|theme| theme.row_colors.get(i % theme.row_colors.len())).cloned()
            ));
        }
        table
    }
    ...
    fn format_row(&self, row: Vec<String>, is_header: bool, row_color: Option<String>) -> String {
        ...
        for cell in &row {
            let formatted_cell = if is_header {
                self.apply_theme(&cell, self.theme.as_ref().and_then(|t| t.header_color.clone()), true)
            } else {
                self.apply_theme(&cell, row_color.clone(), false)
            };
    ...
    fn filter(&self, column: &str, value: &str) -> PyResult<Self> {
        ...
        let filtered_rows: Vec<Vec<Cell>> = self.rows.iter().filter(|row| row[index].value == value).cloned().collect();
        Ok(Table {
...
#[pymodule]
fn rustytable(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_class::<Table>()?;
    m.add_class::<Cell>()?;
    m.add_class::<Theme>()?;
